if (typeof window == "undefined") { // Only run as worker
  ...
  var insideCutsColor = 0x9e6b6b;   // muted rose-grey  — inside/climb cuts
  var outsideCutsColor = 0x6b7f9e;   // cool blue-grey   — outside/conventional cuts
  var pocketColor = 0x7a9e7a;   // sage green-grey  — pocket infill passes
  var toolpathColor = 0x888888;   // neutral mid-grey — toolpath wire preview
  ...
  function getToolpaths(toolpath, jobindex, performanceLimit) {
    ...
    config = {
      ...
      // tabdepth: height the tool lifts to when crossing a tab (above material surface, i.e. stock left)
      ...
      // camTabLocations: array of {x,y} world-space points placed visually by the user
      tabLocations: toolpath.userData.camTabLocations || [],
    ...
    if (config.union == "Yes") {
      ...
      for (i = config.zstart + config.zstep; i < config.zdepth + config.zstep; i += config.zstep) {
        ...
        var drawClipperPathsconfig = {
          ...
          tabLocations: config.tabLocations,
  ...
  drawClipperPathsWithTool = function (config) {
    ...
    var hasTabLocations = config.tabLocations && config.tabLocations.length > 0 &&
      config.tabwidth > 0 && config.tabdepth < 0;

    // ARC-LENGTH TAB INSERTION
    // Tabs span segments correctly, follow path curve at raised Z,
    // and effectiveHalf = tabwidth/2 + toolDia/2 so the cutter does not eat the tab.

    function nearestTOnSeg(px, py, ax, ay, bx, by) {
    ...
    function computeArcLengths(path) {
      ...
      for (var s = 0; s < path.length - 1; s++) {
        a.push(a[s] + distanceFormula(path[s].X, path[s + 1].X, path[s].Y, path[s + 1].Y));
      }
      a.push(a[path.length - 1] +
        distanceFormula(path[path.length - 1].X, path[0].X,
          path[path.length - 1].Y, path[0].Y));
      return a;
    }

    function buildTabEvents(path, arcLen) {
      if (!hasTabLocations) return [];
      var toolR = (config.toolDia > 0 ? config.toolDia : 0) / 2;
      var effHalf = config.tabwidth / 2 + toolR;
      var totalArc = arcLen[path.length];
      var events = [];

      for (var k = 0; k < config.tabLocations.length; k++) {
        var tl = config.tabLocations[k];
        var bestArc = 0, bestDist = Infinity;

        for (var s = 0; s < path.length; s++) {
          var ax = path[s].X, ay = path[s].Y;
          var ns = (s + 1) % path.length;
          var bx = path[ns].X, by = path[ns].Y;
          var t = nearestTOnSeg(tl.x, tl.y, ax, ay, bx, by);
          var nx = ax + t * (bx - ax), ny = ay + t * (by - ay);
          var d = Math.sqrt((nx - tl.x) * (nx - tl.x) + (ny - tl.y) * (ny - tl.y));
          if (d < bestDist) {
            bestDist = d;
            var segStartArc = arcLen[s];
            var segEndArc = (s < path.length - 1) ? arcLen[s + 1] : arcLen[path.length];
            bestArc = segStartArc + t * (segEndArc - segStartArc);
          }
        }

        var liftArc = Math.max(0, bestArc - effHalf);
        var plungeArc = Math.min(totalArc, bestArc + effHalf);
        if (plungeArc > liftArc) {
          events.push({ arc: liftArc, newZ: config.tabdepth });
          events.push({ arc: plungeArc, newZ: config.z });
        }
      }
      events.sort(function (a, b) { return a.arc - b.arc; });
      return events;
    }
    ...
    for (var i = 0; i < config.paths.length; i++) {
      ...
      if (config.leadInPaths) {
        if (config.leadInPaths.length == config.paths.length) {
          ...
          config.leadInPaths[i].sort(function (a, b) {
            return distanceFormula(a.X, config.paths[i][0].X, a.Y, config.paths[i][0].Y) -
              distanceFormula(b.X, config.paths[i][0].X, b.Y, config.paths[i][0].Y);
          });
          lineUnionGeo.vertices.push(new THREE.Vector3(
            config.leadInPaths[i][pointIndex].X,
            config.leadInPaths[i][pointIndex].Y, config.z));
          clipperArr.push({
      ...
      var isDeepestPass = config.z <= config.tabdepth;
      var tabEvents = [];
      var arcLen = [];
      if (hasTabLocations && isDeepestPass) {
        arcLen = computeArcLengths(config.paths[i]);
        tabEvents = buildTabEvents(config.paths[i], arcLen);
      }

      var currentZ = config.z;
      var eventIdx = 0;
      var activeTab = null;

      for (var j = 0; j < config.paths[i].length; j++) {
        var ax = config.paths[i][j].X;
        var ay = config.paths[i][j].Y;
        var isLast = (j === config.paths[i].length - 1);
        var bx = isLast ? config.paths[i][0].X : config.paths[i][j + 1].X;
        var by = isLast ? config.paths[i][0].Y : config.paths[i][j + 1].Y;

        var segStartArc = (arcLen.length > 0) ? arcLen[j] : 0;
        var segEndArc = (arcLen.length > 0)
          ? (isLast ? arcLen[config.paths[i].length] : arcLen[j + 1])
          : 0;
        var segArcLen = segEndArc - segStartArc;

        // Emit vertex A at currentZ
        lineUnionGeo.vertices.push(new THREE.Vector3(ax, ay, currentZ));
        clipperArr.push({ X: ax, Y: ay });
        if (activeTab) activeTab.push({ X: ax, Y: ay });

        // Process all tab events that fall strictly inside this segment
        while (eventIdx < tabEvents.length) {
          var ev = tabEvents[eventIdx];
          if (ev.arc <= segStartArc || ev.arc >= segEndArc) break;

          var tEv = (segArcLen > 1e-10) ? (ev.arc - segStartArc) / segArcLen : 0;
          var ex = ax + tEv * (bx - ax);
          var ey = ay + tEv * (by - ay);

          // Move to event point at current Z
          lineUnionGeo.vertices.push(new THREE.Vector3(ex, ey, currentZ));
          clipperArr.push({ X: ex, Y: ey });
          if (activeTab) activeTab.push({ X: ex, Y: ey });
          ...
          if (currentZ === config.tabdepth) {
            activeTab = [{ X: ex, Y: ey }];
          } else {
            if (activeTab) {
              activeTab.push({ X: ex, Y: ey });
              clipperTabsPaths.push(activeTab);
              activeTab = null;
            }
          }
          eventIdx++;
        }
        // vertex B is emitted by next iteration as its vertex A
      }

      // Close the path
      if (config.paths[i].length > 2) {
        lineUnionGeo.vertices.push(new THREE.Vector3(
          config.paths[i][0].X, config.paths[i][0].Y, currentZ));
        clipperArr.push({ X: config.paths[i][0].X, Y: config.paths[i][0].Y });
        if (activeTab) {
          activeTab.push({ X: config.paths[i][0].X, Y: config.paths[i][0].Y });
          clipperTabsPaths.push(activeTab);
          activeTab = null;
        }
      }
      ...
      var lineUnion = new THREE.Line(lineUnionGeo, lineUnionMat);
      if (config.name) { lineUnion.name = config.name; }
      group.add(lineUnion);
      clipperPaths.push(clipperArr);
      ...
      if (config.z < config.tabdepth) { //draw with tabs
        if (config.toolDia > minimumToolDiaForPreview || config.toolDia < -minimumToolDiaForPreview) { //Dont show for very small offsets, not worth the processing time
          ...
          var lineMesh = this.getMeshLineFromClipperPath({
            ...
            opacity: 1,
            isShowOutline: true,
            color: 0xd4a44c,   // warm amber — tab bridges stand out from the grey toolpath
            caps: "negative"
          });
          lineMesh.position.z = config.z;
  ...
  function workerPocketPath(config) {
    ...
    if (config.offset != 0) {
      ...
      var cutwidth = ((config.offset * 2) * config.stepover); // mm per cut
      ...
      if (config.union == "Yes") {
        ...
        for (i = 0; i < 1000; i++) {
          ...
          if (inflateValUsed > 0) {
            ...
            if (inflatedPaths.length > 0) {
              ...
              if (i % 5 === 0) {
                self.postMessage({ running: true, index: config.index, progress: true, pass: i });
              }
            } else {
              break;
            }
        ...
        pocketGrp.children = pocketGrp.children.reverse(); // Inside Out!
  ...
  var fillColor = 0x7a9e9e;   // muted teal-grey — fill/engrave paths
